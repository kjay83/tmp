...
def is_in_boundary(y1:int, before:int, after:int):
   if before < y1 < after:
      return True
   else:
    return False
...
def list_programming_languages():
   ...
   print("after"+after_year)
   print('before'+before_year)
   ...
   print("in function qualif size=")
   print(qualifying_data.__sizeof__())
   print("in function qualif data=")
   print(qualifying_data)

   return {"programming_languages": qualifying_data}

def list_programming_languages2():
   before_year = request.args.get('before_year') or '30000'
   after_year = request.args.get('after_year') or '0'
   print("after"+after_year)
   print('before'+before_year)
   ...
   qualifying_data = list(
       filter(
           lambda pl: is_in_boundary(pl['publication_year'],int(before_year),int(after_year)),
           in_memory_datastore.values()
       )
   )
   print("in function qualif size=")
   ...
   return jsonify ({"programming_languages": qualifying_data})
